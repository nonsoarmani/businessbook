...
const EditDebtDialog = ({ debt, open, onOpenChange }: EditDebtDialogProps) => {
  ...
  useEffect(() => {
    if (debt && open) {
      form.reset({
        ...
        dateGiven: new Date(debt.dateGiven), // Ensure date is a Date object
        dueDate: new Date(debt.dueDate),     // Ensure date is a Date object
        itemsSold: debt.itemsSold,
      });
    }
  }, [debt, open, form]);

  const onSubmit = (values: DebtFormValues) => {
    const updatedDebt: Partial<Debt> & { id: string } = {
      id: debt.id,
      customerName: values.customerName,
      phone: values.phone,
      originalAmount: values.amountOwed, // When editing, original amount might change if the debt was fully re-evaluated
      amountOwed: values.amountOwed,
      dateGiven: values.dateGiven,
      dueDate: values.dueDate,
      itemsSold: values.itemsSold,
      // Status remains the same unless explicitly changed by other actions (e.g., partial payment)
    };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Edit Debt for {debt.customerName}</DialogTitle>
          <DialogDescription>
            Make changes to this debt record. Click save when you're done.
          </DialogDescription>
        </DialogHeader>
